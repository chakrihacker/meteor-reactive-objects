...
ReactiveObjects.setProperty = function (propObj, propName, mixinObj) {
  ...
  if (!propObj._normalArrays) { propObj._normalArrays = {} }
  ...
  Object.defineProperty(propObj, propName, {
    ...
    set: function (value) {
      ...
      if (!setter.stop) {
        var propValue = setter.value;
        if (ReactiveObjects.isArray(setter.value)) {
          propValue = setter.value;
          console.log('is an array')
        } 
                 
        propObj._reactiveProperties[propName] = propValue;
